...
void FFWorker::workerRPC(int workerindex, uint16_t cmd, const std::string& data, Function<void(const std::string&)> func){
    ...
    getRpc().asyncCall(strServiceName, reqmsg, func);
}
static void RpcCallBack(ScriptArgObjPtr funcOBj, const string& data){
    if (funcOBj && funcOBj->isFunc()){
        ...
        {
            LOGERROR((FFWORKER_LOG, "RpcCallBack exception e=<%s>", e_.what()));
        }
        funcOBj->runFunc(ScriptArgUtil<string>::toValue(retmsg.body));
    }
}
void FFWorker::workerRPCForScript(int workerindex, uint16_t cmd, const std::string& data, ScriptArgObjPtr func){
    workerRPC(workerindex, cmd, data, funcbind(&RpcCallBack, func));
}
void FFWorker::asyncHttp(const std::string& url_, int timeoutsec, Function<void(const std::string&)> func){
    Singleton<HttpMgr>::instance().request(url_, timeoutsec, func);
}
...
void FFWorker::asyncHttpForScript(const std::string& url_, int timeoutsec, ScriptArgObjPtr func){
    asyncHttp(url_, timeoutsec, funcbind(&HttpCallBack, func));
}
