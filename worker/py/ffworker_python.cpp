...
static void py_asyncQueryByCfg(const string& host_, long modid, const string& sql_, PyObject* pFuncArg)
{
    AsyncQueryNameCb cb(pFuncArg);
    DbMgr::instance().asyncQueryByCfg(host_, modid, sql_, cb, &Singleton<FFWorkerPython>::instance().getRpc().getTaskQueue());
}
...
static PyObject* py_query(const string& sql_)
{
    ...
    QueryDBResult ret;
    DbMgr::instance().query(sql_, ret);

    {
        string key = "datas";
        PyObject *k = pytype_traits_t<string>::pyobj_from_cppobj(key);
        PyObject *v = pytype_traits_t<vector<vector<string> > >::pyobj_from_cppobj(ret.dataResult);
        PyDict_SetItem(pyRet, k, v);
        Py_DECREF(k);
        Py_DECREF(v);
    }
    {
        string key = "fields";
        PyObject *k = pytype_traits_t<string>::pyobj_from_cppobj(key);
        PyObject *v = pytype_traits_t<vector<string> >::pyobj_from_cppobj(ret.fieldNames);
        PyDict_SetItem(pyRet, k, v);
        Py_DECREF(k);
        Py_DECREF(v);
    }
    {
        string key = "errinfo";
        PyObject *k = pytype_traits_t<string>::pyobj_from_cppobj(key);
        PyObject *v = pytype_traits_t<string>::pyobj_from_cppobj(ret.errinfo);
        PyDict_SetItem(pyRet, k, v);
        Py_DECREF(k);
        Py_DECREF(v);
    }
    {
        string key = "affectedRows";
        PyObject *k = pytype_traits_t<string>::pyobj_from_cppobj(key);
        PyObject *v = pytype_traits_t<int>::pyobj_from_cppobj(ret.affectedRows);
        PyDict_SetItem(pyRet, k, v);
        Py_DECREF(k);
        Py_DECREF(v);
    }
...
static PyObject* py_queryByCfg(const string& host_, const string& sql_)
{
    PyObject* pyRet = PyDict_New();

    QueryDBResult ret;
    DbMgr::instance().queryByCfg(host_, sql_, ret);
...
struct PyWorkerCallBack
{
    PyWorkerCallBack(PyObject* pFuncSrc):pFunc(pFuncSrc){
        if (pFunc != NULL)
        {
            Py_INCREF(pFunc);
        }
    ...
    void operator()(std::string& ret){
        call_python(pFunc, ret);
    }
    void call_python(PyObject* pFuncSrc, std::string ret)
    {
        if (pFuncSrc == NULL)
        {
            return;
        ...
        PyObject *pyRet = pytype_traits_t<string>::pyobj_from_cppobj(ret);
        try
        {
            if (Singleton<FFWorkerPython>::instance().m_enable_call)
            {
        ...
        {
            LOGERROR((FFWORKER_PYTHON, "workerobj_python_t::call_service exception<%s>", e_.what()));
        }
        Py_XDECREF(pFuncSrc);
        Py_DECREF(pyRet);
    }
...
{
    Singleton<FFWorkerPython>::instance().workerRPC(workerindex, cmd, argdata, PyWorkerCallBack(pFuncArg));
}
struct PyAsyncHttpCb
{
    PyAsyncHttpCb(PyObject* pFuncSrc):pFunc(pFuncSrc){
        if (pFunc != NULL)
        {
            Py_INCREF(pFunc);
        }
    ...
    void call_python(PyObject* pFuncSrc, std::string ret)
    {
        ...
        {
            LOGERROR((FFWORKER_PYTHON, "workerobj_python_t::py_asyncHttp exception<%s>", e_.what()));
        }
        Py_XDECREF(pFuncSrc);
        Py_DECREF(pyRet);
    }
...
{
    Singleton<FFWorkerPython>::instance().asyncHttp(url_, timeoutsec, PyAsyncHttpCb(pFuncSrc));
    return true;
}
...
int FFWorkerPython::workerInit(const string& py_root)
{
    string path;
    ...
    LOGTRACE((FFWORKER_PYTHON, "FFWorkerPython::workerInit begin path:%s, m_ext_name:%s", path, m_ext_name));
    ...
    (*m_ffpython).reg(&FFDb::escape, "escape")
                 ...
                 .reg(&py_query, "query")
                 .reg(&py_asyncQueryByCfg, "asyncQueryByCfg")
                 .reg(&py_queryByCfg, "queryByCfg")
...
void FFWorkerPython::workerCleanup()
{
    try
    {
    ...
    {
        LOGERROR((FFWORKER_PYTHON, "workerCleanup failed er=<%s>", e_.what()));
    }
    this->cleanupModule();
    m_enable_call = false;
    DbMgr::instance().cleanup();
    LOGINFO((FFWORKER_PYTHON, "FFWorkerPython::workerCleanup end"));
}
int FFWorkerPython::close()
{
    getRpc().getTaskQueue().post(funcbind(&FFWorkerPython::workerCleanup, this));
    FFWorker::close();
    if (false == m_started)
        return 0;
    m_started = false;
